...
# Get OAuth endpoints from discovery (we'll fetch these dynamically)
...
try:
    ...
    revocation_endpoint = discovery_data.get("revocation_endpoint")
    ...
    revocation_endpoint = f"{issuer_url}/protocol/openid-connect/revoke"
    jwks_uri = f"{issuer_url}/protocol/openid-connect/certs"

# Client credentials
client_id = os.environ.get("OAUTH_CLIENT_ID", "mcp")
client_secret = os.environ.get("OAUTH_CLIENT_SECRET", "KlUZZ3t6T8qDYdB4JxJzhKKe2FMfLJuY")

# Get base URL for this server (where it's hosted)
base_url = os.environ.get("MCP_BASE_URL", "http://localhost:3443")

# Create JWT verifier to validate tokens
jwt_verifier = JWTVerifier(
    jwks_uri=jwks_uri,
    audience=client_id,  # The audience should be the client_id
    issuer=issuer_url,
)
...
print(f"  Valid Scopes: openid, profile, email")
